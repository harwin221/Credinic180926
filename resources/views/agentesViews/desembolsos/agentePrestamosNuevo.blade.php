...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
{{--                    campo para saber el cliente que se seleccionó--}}
                    <input type="hidden" value="{{request()->get('cliente')}}" id="clienteRequest">
                    {{html()->form()->method('POST')->action(route('agentes.user.desembolso.store'))->id('frmPrestamo')->open()}}
                    <div class="row">
                                            ...
                                            <div id="datosClienteStep" class="content" role="tabpanel" aria-labelledby="datosClienteId">
                                                ...
                                                <div class="row">
                                                    <div class="col-md-12">
                                                        ...
                                                        {{html()->select('cliente',[],null)->class('form-control text-center select2')->style(['width'=>'100%'])->id('selectClientes')->required()}}
                                                    </div>
                                                </div>
                                                <br>
...
@section('script')
    <script>
        ...
        async function getListClientes() {
            ...
            let url = '{{route('agente.user.clientes.getListClientes')}}'
            ...
            const clienteRequest = document.getElementById('clienteRequest').value
            if (clienteRequest !== '') {
                $("#selectClientes").val(clienteRequest).trigger('change')
            }
        }
        ...
        async function getListNegociosCliente(clienteID) {
            ...
            let url = '{{route('agente.negocio.getListNegociosCliente','?')}}'
        ...
        async function getDatosCliente(clienteID) {
            let url = '{{route('agente.clientes.getDatosCliente','?')}}'
            url = url.replace('?', clienteID)
            const {data} = await axios.get(url)
        ...
        async function getDatosNegocio(negocioID) {
            ...
            let url = '{{route('agente.negocio.getInfoNegocio','?')}}'
        ...
        async function saveClienteNegocio(){
            ...
            let url = '{{route('agente.negocio.storeNegocioCliente','?')}}'
            url = url.replace('?', clienteId)
        ...
        async function getTiposNegocios() {
            const {data} = await axios.get('{{route('agente.configuracion.tiposNegocios.getListTiposNegocios')}}')
            for (let key in data) {
                let opcion = `<option value="${key}"> ${data[key]} </option>`
        ...
        async function getFiadoresUser(clienteId) {
            ...
            let url = '{{route('agente.fiador.getListFiadoresUser','?')}}'
            url = url.replace('?', clienteId)
        ...
        async function getDatosFiador(fiadorID) {
            ...
            let url = '{{route('agente.fiador.getDatos','?')}}'
        ...
        async function saveFiadorNegocio(){
            ...
            let url = '{{route('agente.fiador.storeFiador','?')}}'
            url = url.replace('?', clienteId)
            ...
            if (montoIntereses.value !== '' && montoTotalFinanciar.value !== '' && montoCuota.value !== '') {
                ...
                for (let i=0;i <= resPlazo ;i++)
                {
                    if (i === 0) {
                        ...
                        let mesCopia = parseInt(fechaCopia.toLocaleString().split('/')[1]) - 1 //meses desde el 0 ENERO al 11 diciembre
                        ...
                        if(diasPreferidos > 0 && formaPago === "3")//no se ingreso dias preferidos
                            console.log()
                        else {
        ...
        function resumenPrestamo(){
            ...
            document.getElementById('resumenVendedor').innerText = $("#selectVendedor").val()
            document.getElementById('resumenCobrador').innerText = $("#selectAgente").val()
            document.getElementById('resumenDesembolso').innerText = $("#selectDesembolso").val()
        ...
        async function savePrestamo() {
            if (!validarCamposPrestamo()) {
                ...
                const {data} = await axios.post('{{route('agentes.user.desembolso.store')}}', frmData)
                Swal.fire({
                    title: 'Estado',
                    icon: 'success',
                    text: data.message,
                ...
                setTimeout(() => {
                    window.location = '{{route('agentes.user.clientes.index')}}'
                }, 2000)
            } else
