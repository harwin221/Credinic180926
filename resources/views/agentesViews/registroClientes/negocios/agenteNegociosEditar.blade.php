...
@section('tituloPagina')
    @include('utlisComponents.btnBack',['url'=>route('agentes.user.clientes.edit',encode($user_negocio->user_id))])
    Información del Negocio [ <span style="color: blue;font-weight: bold">{{$user_negocio->user->full_name}}</span> ]
@endsection
@section('content')
    {{html()->form('POST',route('agentes.user.negocios.update',$user_negocio->id_enc))->acceptsFiles()->open()}}
    @method('PUT')
    <div class="row">
        <div class="col-md-12">
