...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <a href="{{route('agentes.indexNuevoCliente')}}" class="btn btn-primary btn-sm">Crear Nuevo Cliente <i class="fa fa-plus"></i></a>
            <a href="#" class="btn btn-success btn-sm">Solcitud de Cliente Existente <i class="fa fa-plus"></i></a>
        </div>
    </div>
    <br>
    <div class="row">
                ...
                <table class="table table-sm table-striped" style="font-size: 11px">
                    ...
                    @foreach($clientes as $cliente)
                        <tr>
                            ...
                            <td>{{$cliente->departamento_municipio->departamento->nombre." / ".$cliente->departamento_municipio->nombre}}</td>
                            <td>{{$cliente->telefono1." / ".$cliente->telefono2}}</td>
                            <td>
                                <a href="{{route('agentes.editNuevoCliente',$cliente->id_enc)}}" class="btn btn-sm btn-primary w-100 mb-1">Ver</a>
                                {{html()->form('POST',route('agentes.destroyNuevoCliente',$cliente->id_enc))->open()}}
                                <a href="#" class="btn btn-sm btn-danger w-100">Eliminar</a>
                                {{html()->form()->close()}}
                            </td>
                        </tr>
                    @endforeach
...
<script>
    ...
    async function buscarClientes(buscar) {
        try {
            ...
            const response = await axios.get('{{route('agentes.registroClientes')}}', {
